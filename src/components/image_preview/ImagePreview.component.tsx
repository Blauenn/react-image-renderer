					...
					<div className="absolute top-0 left-0 flex flex-col justify-between h-full">
						...
						<div className="flex flex-col gap-24">
							...
							<div className="flex flex-col gap-4">
								...
								<h1 className={`text-[56px] font-bold ${cameraInformation.lens_make_model.endsWith("G Master") && otherInfo.lens_highlight ? "text-[#cf4019]" : null}`}>
									{cameraInformation.lens_make_model}
								</h1>
							</div>
							{/* Camera settings */}
							...
							<div className="flex flex-col gap-16 mt-8">
								...
								<div className="flex flex-col gap-12">
									{contactInfo.facebook != "" ? (
										<div className="flex flex-row items-center gap-8">
											...
											<h1 className="text-[40px] opacity-50 font-semibold">{contactInfo.facebook}</h1>
										</div>
									) : null}
									{contactInfo.instagram != "" ? (
										<div className="flex flex-row items-center gap-8">
											<img src="/assets/instagram.png" className="w-[60px] h-[60px]" />
											<h1 className="text-[40px] opacity-50 font-semibold">{contactInfo.instagram}</h1>
										</div>
									) : null}
								</div>
								{/* Second person */}
								<div className="flex flex-col gap-12">
									{contactInfo.facebook_2 != "" ? (
										<div className="flex flex-row items-center gap-8">
											<img src="/assets/facebook.png" className="w-[60px] h-[60px]" />
											<h1 className="text-[40px] opacity-50 font-semibold">{contactInfo.facebook_2}</h1>
										</div>
									) : null}
									{contactInfo.instagram_2 != "" ? (
										<div className="flex flex-row items-center gap-8">
											<img src="/assets/instagram.png" className="w-[60px] h-[60px]" />
											<h1 className="text-[40px] opacity-50 font-semibold">{contactInfo.instagram_2}</h1>
										</div>
									) : null}
								</div>
							</div>
